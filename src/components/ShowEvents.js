...
const ShowEvents = () => {
  ...
  const handleEditClick = (event) => {
    ...
    setEditFormData({
      ...
      EventID: event.EventID,
      EventDate: event.EventDate,
      EventLocation: event.EventLocation,  // Populate new field
      EventOrganizer: event.EventOrganizer, // Populate new field
    });
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setEditingId(null);
      setEditFormData({ EventName: '', EventID: '', EventDate: '', EventLocation: '', EventOrganizer: '' });
    } catch (error) {
      console.error('Error updating event:', error);
      Swal.fire('Error!', 'Failed to update event.', 'error');
    }
        ...
        <tbody>
          {events.map((event) => (
            <tr key={event.$id}>
              ...
              <td>{event.EventID}</td>
              <td>{event.EventDate}</td>
              <td>{event.EventLocation}</td>  {/* Display new field */}
              <td>{event.EventOrganizer}</td> {/* Display new field */}
              <td>
                <button onClick={() => handleEditClick(event)}>Edit</button>
                <button onClick={() => handleDelete(event.$id)}>Delete</button>
              </td>
